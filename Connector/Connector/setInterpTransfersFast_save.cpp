...
void K_CONNECTOR::init_TransferInter(
...
void K_CONNECTOR::del_TransferInter(
    std::pair<RecvQueue*, SendQueue*>*& pair_of_queue_loc) {
...
//=============================================================================
void K_CONNECTOR::setInterpTransfersFast(
  E_Float**& iptro_tmp, E_Int& nvars         , E_Int*& param_int_tc, E_Float*& param_real_tc , E_Int**& param_int     , E_Float**& param_real, E_Int*& ipt_omp,
...
/*  cout << "Inside setInterpTransfersFast ";*/
#ifdef _MPI
 ...
 //Swap (call to setInterpTransfer)
  if ( (param_int_tc != NULL) && (param_real_tc != NULL))
  {
      ...
      if (pair_of_queue  != NULL)
        {
          ...
          del_TransferInter(pair_of_queue_IBC);
    ...
    {
      if (pair_of_queue     == NULL) { init_TransferInter(pair_of_queue    );}
      if (pair_of_queue_IBC == NULL) { init_TransferInter(pair_of_queue_IBC);}
...
/*      int nb_send_buffer = 0;
      for (E_Int ip2p = 1; ip2p < param_int_tc[1]+1; ++ip2p)
      {
        ...
        if (dest != rank)  // Inter Process ibc
        {
          ...
          setInterpTransfersInter(iptro_tmp    , nvars      , param_int_tc, param_real_tc,
...
/*
    ...
    for (E_Int ip2p = 1; ip2p < param_int_tc[1]+1; ++ip2p)
    {
      ...
      { TypeTransfert = 1;
        ...
        K_CONNECTOR::setInterpTransfersIntra(iptro_tmp    , nvars    , param_int_tc, param_real_tc,
                                         param_int    , param_real , ipt_omp     , linelets_int, linelets_real, TypeTransfert, it_target, nidom, ip2p, 
...
/*     if (mpi )
    {
      ...
      //
      getTransfersInter(nbcomIBC_S, iptro_tmp, param_int, param_int_tc , pair_of_queue_IBC);
...
*/
    ...
    for (E_Int ip2p = 1; ip2p < param_int_tc[1]+1; ++ip2p)
    {
      ...
      if (dest == rank)  // Intra Process Id
      {
        TypeTransfert = 0;
        cout << "Intra com " << dest << "\n";
        setInterpTransfersIntra(iptro_tmp, nvars   , param_int_tc, param_real_tc ,
                                         param_int, param_real,  ipt_omp    ,linelets_int, linelets_real, TypeTransfert , it_target, nidom, ip2p, 
                                         ipt_timecount, nstep, nitmax, rk, exploc, numpassage); 
      }    
    }
    ...
    if (mpi)
    {
       ...
       getTransfersInter(nbcomID, iptro_tmp, param_int, param_int_tc , pair_of_queue);
...
// Idem: in place + from zone + tc compact au niveau base
//=============================================================================
void K_CONNECTOR::setInterpTransfersIntra(

    E_Float**& ipt_ro, E_Int& nvars, E_Int*& param_int_tc,
	 ...
	 if (autorisation_transferts[pass_inst][irac_auto]==1)
	  {
          ...
          //cout << "nvars_loc is " << nvars_loc << " nvars is " << nvars << "\n";
          cout << "NoD is " << NoD << " NoR is " << NoR << "\n";
...
void K_CONNECTOR::setInterpTransfersInter(
    E_Float**& ipt_ro   , E_Int& nvars   , E_Int*& param_int_tc, E_Float*& param_real_tc,
...
/*                     if (nvars_loc == 5 || nvars_loc == 6) // Transferts NS classiques ou LBM -> NS
                     {*/
                      for (E_Int eq = 0; eq < nvars_loc; eq++) { vectOfDnrFields[eq] = ipt_ro[NoD]    + eq * param_int[NoD][ NDIMDX];}// printf("NDIMDX is %d %d \n",NoD,param_int[NoD][ NDIMDX]);}
/*                     }*/
/*                     else */
/*                     if (nvars_loc == param_int[NoD][NEQ_LBM] + 5) // Transferts LBM classiques
                     {
...
// in place + from zone + tc compact
//=============================================================================
void K_CONNECTOR::getTransfersInter( E_Int& nbcom, E_Float**& ipt_ro, E_Int**& param_int, E_Int*& param_int_tc, std::pair<RecvQueue*, SendQueue*>*& pair_of_queue_loc) {
...
/*           printf("Nozone Verif= %d %d %d %d \n", recv_nozone[irac],recv_size[irac], irac, recv_nvarloc[irac] );*/
         }
...
#pragma omp parallel
         {
          ...
          { 
            ...
            if (NoR < 0) decal=0;
/*            cout << "Receive Data " << recv_nvarloc[irac];*/
