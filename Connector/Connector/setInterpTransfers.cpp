...
/*      cout<< "dirR "<< dirR << " initAll " << initAll <<" \n";*/
      E_Int posvx=-1, posvy=-1, posvz=-1, posmx=-1, posmy=-1, posmz=-1;
      if ( dirR > 0 )
	{
	  posvx = K_ARRAY::isNamePresent("VelocityX", varStringR);
...
PyObject* K_CONNECTOR::___setInterpTransfers(PyObject* self, PyObject* args)
{
  ...
  if (!PYPARSETUPLE(args,
                    ...
                    "OOOOOllllllllll", 
                    "OOOOOiiiiiiiiii",
                    &zonesR, &zonesD, &pyVariables, &pyParam_int,  
...
/*  cout << " Inside ___setInterpTransfers "; */ 
...
/*  cout << " Inside ___setInterpTransfers 2 " << varname << "\n";*/
...
/*  cout << " Inside ___setInterpTransfers 3\n";*/
...
/*  E_Int nssiter = iptdtloc[0];
  ...
  for (E_Int nd = 0; nd < nidomR; nd++) {impli_local[nd]=1;}
		...
		if (autorisation_transferts[pass_inst][irac_auto]==1)
		  {
			...
			imd= ipt_param_intR[ NoD ][ NIJK ] + 2*ipt_param_intR[ NoD ][ 0 ]; jmd= ipt_param_intR[ NoD ][ NIJK+1] + 2*ipt_param_intR[ NoD ][ 0 ];
